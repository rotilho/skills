...
Use `~/IdeaProjects/skills` for global skills when the procedure is reusable across multiple repos, tools, agents, or workflows.

Use `<target-repo>/.agents/skills` for repo-local skills when the procedure depends on one repo's structure, scripts, product language, deployment topology, test fixtures, or ownership boundaries.
...
- `npx skills add ~/IdeaProjects/skills/ -g --all -y` was run after global changes
